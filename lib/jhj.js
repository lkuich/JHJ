...
const injectSocket = (head) => {
  ...
  head.appendChild(parse(socketIOBody))
};
...
const parseRoot = async (index) => {
  ...
  const templates = await parseTemplate(root, head);
...
const parseIndex = async (index) => {
  const root = await parse(index);

  const codeBlocks = await parseServerSideJs(root);
