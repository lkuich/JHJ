...
const parseRoot = async (index) => {
  const { root, codeBlocks } = await parseIndex(index);
  const body = root.querySelector('body');

  const templates = await parseTemplate(root, body);
  
  const finalCodeBlocks = [...codeBlocks, ...templates.codeBlocks];


  injectSocket(body);

  return { component: templates.component, codeBlocks: finalCodeBlocks };
};

const parseIndex = async (index) => {
  const codeBlocks = [];

  const root = parse(index);

  codeBlocks.push(...(await parseServerSideJs(root)));

  return { root, codeBlocks };
}
...
export const renderJsj = async (file) => {
...
export const socketPickup = (socket, serverCode) => {
  try {
    // const modules = new Function(serverCode.join(''));
    // modules()
